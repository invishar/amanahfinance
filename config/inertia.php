...
|--------------------------------------------------------------------------
|
| Config bawaan paket, dengan tiga penyesuaian khusus repo ini:
|
| 1. `ssr.bundle` ditunjuk eksplisit ke bootstrap/ssr/ssr.js (output
|    `vite build --ssr`, lihat vite.config.js) supaya `inertia:start-ssr`
|    tidak menebak-nebak dan pesan errornya jelas kalau bundle belum dibuild.
| 2. `ssr.enabled` default true, tapi saat `npm run dev` (Vite hot) SSR
|    dimatikan lewat `Inertia::disableSsr()` di AppServiceProvider -- dev
|    server Vite di sini tidak menyediakan endpoint /__inertia_ssr.
| 3. `pages.paths` memakai huruf besar (resources/js/Pages), sesuai penamaan
|    folder yang dipakai app.tsx dan routes/web.php.
|
| Kalau proses SSR mati/tidak jalan (kasus normal di hPanel: lihat README),
| Inertia diam-diam jatuh balik ke render sisi klien -- halaman tetap hidup,
| cuma HTML awalnya kosong seperti sebelum SSR dipasang.
|
*/
